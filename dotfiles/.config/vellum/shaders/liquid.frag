...
void main() {
    ...
    vec3 q = vec3(fbm(p3), fbm(p3 + vec3(5.2, 1.3, t * 0.2)), 0.0);
    vec3 r = vec3(fbm(p3 + 2.0 * q + vec3(1.7, 9.2, t * 0.5)), fbm(p3 + 2.0 * q + vec3(8.3, 2.8, t * -0.5)), 0.0);
    
    float val = fbm(p3 + 3.0 * r);
    val = val * 0.5 + 0.5;
    ...
    for(int i = 0; i < NUM_COLORS; i++) {
        ...
        float softness = max(0.01, min(iBlur, weight * iEdgeBlur) * 2.0);
        
        float startMask = (i == 0) ? 1.0 : smoothstep(cumulative - softness, cumulative + softness, val);
        float endMask = (i == iCount - 1) ? 0.0 : smoothstep(nextCumulative - softness, nextCumulative + softness, val);
        
        finalOklab += iColorsOklab[i] * max(0.0, startMask - endMask);
