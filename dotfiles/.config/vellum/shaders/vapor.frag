...
void main() {
    ...
    vec3 finalOklab = iColorsOklab[0];
    float totalWeight = iRatios[0];

    for (int i = 1; i < 6; i++) {
        if (i >= limit) break;
        
        float fi = float(i);
        vec3 p3 = vec3(p * (iZoom - fi * 0.1), t * (0.8 + 0.1 * fi) + fi * 13.37);
        
        float n = fbm(p3);
        n = n * 0.5 + 0.5;
        
        float edge = max(0.01, iEdgeBlur);
        n = smoothstep(0.5 - edge, 0.5 + edge, n);
        
        float w = n * max(iRatios[i], 0.1) * (1.0 + iBlur);
        finalOklab = mix(finalOklab, iColorsOklab[i], w / (totalWeight + w + 0.0001));
        totalWeight += w;
    }
