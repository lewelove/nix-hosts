  ...
  systemd.services.awg-vpn = {
    ...
    serviceConfig = {
      ...
      ExecStopPost = pkgs.writeShellScript "awg-post-stop" ''
        ${pkgs.iproute2}/bin/ip rule del to 100.64.0.0/10 lookup 52 priority 500 || true
        ${pkgs.iptables}/bin/iptables -t mangle -D FORWARD -o active -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu || true
      '';
    };
  };
